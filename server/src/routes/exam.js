...
router.post('/', function(req, res, next) {
	...
	roomDb.findOne({"desks.browserIP":browserIP},function(err,room){
	    if(!room){//如果没有找到考场，说明读卡器未绑定，提示客户端绑定读卡器
	    	obj.msg='bindReader';
	    	res.json(obj);
	    	return;
	    }//否则，找到考场
	    ...
	    banjiDb.findOne({"students.card":obj.cardID},function(err,banji){
		    if(!banji){//如果没有找到班级，说明学生卡未绑定，提示客户端绑定学生卡
		    	obj.msg='bindStudent';
		    	res.json(obj);
		    	return;
		    }//否则，找到班级
		    //查找学生信息
		    const student=findStudent(banji.students,obj.cardID);
		    //查看当前是否有考试
		    examDb.find({"room_id":room._id},function(err,exams){
		    	const exam=findExam(exams);
			    if(!exam){//如果没有找到考试，提示没有考试
			    	obj.msg='noexam';
