...
router.get('/:examID/:studentID', function(req, res, next) {
	...
	answersheetDb.findOne({examID:examID,studentID:studentID},function(err,answersheet){
		...
		examDb.findOne({_id:examID},function(err,exam){
			...
			tree.read_node(exam.paper_gid).then(paper=>{
				const questions=paper._data.data.questions;
				var prices=paper._data.data.scores;//题目的分值，为了与学生得分区别，用price表示 
				const shuffled_questions=_.shuffle(questions);//乱序
				console.log('group_questions',shuffled_questions)
				tree.read_nodes(shuffled_questions).then(nodes=>{
					const group_questions=_.groupBy(nodes,function(obj){return obj._data.type});
					// console.log('group_questions',group_questions);
					const gq=group_questions;
					const ordered_questions=_.concat(gq['ks1/choice']||[], gq['ks1/tf']||[],gq['ks1/revise']||[],gq['ks1/qa']||[]);
					// console.log('ordered_questions',ordered_questions);
					const ordered_questions_id=_.map(ordered_questions, '_id');
				    ...
				    const answersheet={examID:examID,examName:exam.name,studentName:student.name,studentID:studentID,questions:ordered_questions_id,answers:{},prices:prices,begin:new Date().getTime()};
