			...
			tree.read(exam.paper_id).then(function (node) {
				...
				tree.read_nodes(shuffled_questions).then(function (nodes) {
					...
					banjiDb.findOne({ "students.id": studentID }, function (err, banji) {
						...
						var answersheet = { examID: examID, examName: exam.name, studentName: student.name, studentID: studentID, questions: ordered_questions_id, answers: {}, begin: new Date().getTime() };
