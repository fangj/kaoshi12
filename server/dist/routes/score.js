...
function score(answersheetID, cb) {
	answersheetDb.findOne({ _id: answersheetID }, function (err, answersheet) {
		...
		examDb.findOne({ _id: answersheet.examID }, function (err, exam) {
			...
			tree.read_nodes(answersheet.questions).then(function (questions) {
				tree.read_node(exam.paper_gid).then(function (paperNode) {
...
function isCorrect(qnode, answer) {
	...
	switch (qnode._data.type) {
		case "ks1/tf":
			return isCorrectTf(data, answer);
		case "ks1/choice":
			return isCorrectChoice(data, answer);
		default:
			return false;
	}
}
